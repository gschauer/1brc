...
if [ ! -f target/CalculateAverage_gschauer_image ]; then
    ...
    # NATIVE_IMAGE_OPTS="$NATIVE_IMAGE_OPTS -H:MethodFilter=CalculateAverage_gschauer.* -H:Dump=:2 -H:PrintGraph=Network"
    
    native-image $NATIVE_IMAGE_OPTS -cp target/average-1.0.0-SNAPSHOT.jar -o target/CalculateAverage_gschauer_image dev.morling.onebrc.CalculateAverage_gschauer
fi
